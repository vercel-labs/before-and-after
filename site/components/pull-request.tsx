"use client"

import { useState } from "react"
import { Browser } from "@/components/browser"

// GitHub PR icon in green
function GitPullRequestIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M7.17677 3.07322L9.57322 0.676753C9.73071 0.51926 9.99999 0.630802 9.99999 0.853529V5.64642C9.99999 5.86915 9.73071 5.98069 9.57322 5.8232L7.17677 3.42677C7.07914 3.32914 7.07914 3.17085 7.17677 3.07322ZM3.25 2.5C2.83579 2.5 2.5 2.83579 2.5 3.25C2.5 3.66421 2.83579 4 3.25 4C3.66421 4 4 3.66421 4 3.25C4 2.83579 3.66421 2.5 3.25 2.5ZM1 3.25C1 2.00736 2.00736 1 3.25 1C4.49264 1 5.5 2.00736 5.5 3.25C5.5 4.22966 4.87389 5.06309 4 5.37197V10.628C4.87389 10.9369 5.5 11.7703 5.5 12.75C5.5 13.9926 4.49264 15 3.25 15C2.00736 15 1 13.9926 1 12.75C1 11.7703 1.62611 10.9369 2.5 10.628V5.37197C1.62611 5.06309 1 4.22966 1 3.25ZM11 2.5H10V4H11C11.5523 4 12 4.44772 12 5V10.628C11.1261 10.9369 10.5 11.7703 10.5 12.75C10.5 13.9926 11.5074 15 12.75 15C13.9926 15 15 13.9926 15 12.75C15 11.7703 14.3739 10.9369 13.5 10.628V5C13.5 3.61929 12.3807 2.5 11 2.5ZM2.5 12.75C2.5 12.3358 2.83579 12 3.25 12C3.66421 12 4 12.3358 4 12.75C4 13.1642 3.66421 13.5 3.25 13.5C2.83579 13.5 2.5 13.1642 2.5 12.75ZM12 12.75C12 12.3358 12.3358 12 12.75 12C13.1642 12 13.5 12.3358 13.5 12.75C13.5 13.1642 13.1642 13.5 12.75 13.5C12.3358 13.5 12 13.1642 12 12.75Z"
        fill="#1B7F38"
      />
    </svg>
  )
}

const USERNAME = "juangabriel"

interface PullRequestProps {
  tab?: "write" | "preview"
  defaultTab?: "write" | "preview"
  onTabChange?: (tab: "write" | "preview") => void
  markdown?: string
  className?: string
  interactive?: boolean
}

export function PullRequest({
  tab: controlledTab,
  defaultTab = "preview",
  onTabChange,
  markdown = DEFAULT_MARKDOWN,
  className,
  interactive = true,
}: PullRequestProps) {
  const [internalTab, setInternalTab] = useState<"write" | "preview">(controlledTab ?? defaultTab)
  const [prevControlledTab, setPrevControlledTab] = useState(controlledTab)

  // Sync internal state when controlled value changes (during render, not in effect)
  if (controlledTab !== prevControlledTab) {
    setPrevControlledTab(controlledTab)
    if (controlledTab !== undefined) {
      setInternalTab(controlledTab)
    }
  }

  const handleTabClick = (newTab: "write" | "preview") => {
    setInternalTab(newTab)
    onTabChange?.(newTab)
  }

  const tab = internalTab

  return (
    <div className={`w-full relative ${className ?? ""}`}>
      {/* Main content box */}
      <div className="w-full">
        {/* Top nav bar with username */}
        <div className="bg-[--geist-blue-300] border border-[--geist-blue-500] rounded-t-md px-2 py-1 flex items-center gap-1">
          <GitPullRequestIcon />
          <span className="text-[10px] font-semibold text-neutral-800">{USERNAME}</span>
        </div>

        {/* Tab bar */}
        <div className="bg-neutral-100 border-x border-neutral-200 relative">
          <div className={`flex ${interactive ? "" : "pointer-events-none"}`}>
            <button
              type="button"
              onClick={() => handleTabClick("write")}
              className={`px-2 py-1 text-[9px] relative cursor-pointer border rounded-t -mb-px ml-1.5 mt-1 ${
                tab === "write"
                  ? "bg-white text-neutral-800 border-neutral-200 border-b-white"
                  : "text-neutral-500 border-transparent hover:text-neutral-700"
              }`}
            >
              Write
            </button>
            <button
              type="button"
              onClick={() => handleTabClick("preview")}
              className={`px-2 py-1 text-[9px] relative cursor-pointer border rounded-t -mb-px ml-0.5 mt-1 ${
                tab === "preview"
                  ? "bg-white text-neutral-800 border-neutral-200 border-b-white"
                  : "text-neutral-500 border-transparent hover:text-neutral-700"
              }`}
            >
              Preview
            </button>
          </div>
          <div className="border-b border-neutral-200" />
        </div>

        {/* Content area - preview always controls height, write is absolutely positioned */}
        <div className="border border-t-0 border-neutral-200 rounded-b-md bg-white p-2 relative overflow-hidden">
          {/* Write content - absolutely positioned so it doesn't affect height */}
          <div className={`absolute inset-0 p-2 ${tab === "write" ? "" : "invisible"}`}>
            <WriteContent markdown={markdown} />
          </div>
          {/* Preview content - always in flow to control height */}
          <div className={tab === "preview" ? "" : "invisible"}>
            <PreviewContent />
          </div>
        </div>
      </div>
    </div>
  )
}

// Write tab content with markdown
export function WriteContent({ markdown = "" }: { markdown?: string }) {
  return (
    <div className="font-mono text-[7px] text-neutral-500 whitespace-pre leading-tight">
      {markdown}
    </div>
  )
}

// Preview tab content with title and before/after table using Browser components
export function PreviewContent() {
  return (
    <div>
      <h1 className="text-[10px] font-semibold text-neutral-800 mb-2">Update the homepage</h1>
      <table className="w-full border-collapse text-[7px] table-fixed">
        <thead>
          <tr>
            <th className="w-1/2 py-1 px-1 text-center text-neutral-700 font-medium border border-neutral-200">Before</th>
            <th className="w-1/2 py-1 px-1 text-center text-neutral-700 font-medium border border-neutral-200">After</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td className="p-1 border border-neutral-200 align-top">
              <div className="w-full">
                <Browser variant="A" url="site.com" />
              </div>
            </td>
            <td className="p-1 border border-neutral-200 align-top">
              <div className="w-full">
                <Browser variant="B" url="site.com" />
              </div>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
  )
}

// Default title for PR description
export const DEFAULT_TITLE = "# Update the homepage"

// Default markdown for PR description
export const DEFAULT_MARKDOWN = `${DEFAULT_TITLE}

| Before | After |
|:------:|:-----:|
| ![Before](a.png) | ![After](b.png) |`
